...
class MonitorFactory:
    ...
    @staticmethod
    def create_monitor(interface: str, interval: int = 5, ports: Optional[Set[int]] = None) -> NetworkMonitor:
        try:
            monitor = NetworkMonitor(interface, interval, ports)
    ...
    @staticmethod
    def create_monitors_from_config(config: List[Dict]) -> List[NetworkMonitor]:
        monitors = []
        for monitor_config in config:
            ...
            monitor = MonitorFactory.create_monitor(interface, interval, ports)
